...
def create_GoC_network( duration = 2000, dt=0.025, seed = 123, runid=0, run=False, minI=-75, maxI=200, iStep=25, iDur=400, iRest=500, girk=False ):
	"""
	Create a network of GoCs and inject current of varying amplitude in 1 GoC to measure its response while embedded in an electrically coupled network

	Relevant parameters:
	===================
	minI : minimum current amplitude in nA
	maxI : maximum current amplitude in nA
	iStep: how many levels of current amplitudes between minI and maxI (inclusive)
	iDur:  duration of current Injection (in ms)
	iRest: baseline period between different injections (in ms)
	girk: include girk channel? (Boolean)
	"""
	wGirk=''
	if girk:
		wGirk ='_wGIRK'
	...
	goc_filename = 'Golgi/{}.cell.nml'.format(gocID)
	...
	ctr=0
	goc=0
	p = {"iAmp": np.arange(minI, maxI+iStep/2, iStep), "iDuration": iDur, "iRest": iRest}
