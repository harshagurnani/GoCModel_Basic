...
def create_GoC( runid=0, usefile='cellparams_file.pkl', girk=0, morpho_fname='Golgi_reduced_twoCaPools.cell.nml', has2Pools=True, add2Pools=True ):
	"""
	...
	girk: 	Density of GIRK channels (0 for no girk)
	morpho_fname: filename (NeuroML file) with example cell whose morphology will be imported
	has2Pools:  (Boolean) whether morphology file is of class Cell2CaPools (if False, assumes it is Cell)
	add2Pools:  (Boolean) whether make class Cell2CaPools or Cell
	"""
	...
	if add2Pools:
		gocID = 'GoC_2Pools_'+format(runid, '05d')+'_girk_'+format(int(1e3*girk))
	...
	if has2Pools:
		morpho = morpho_file.cell2_ca_poolses[0].morphology
	else:
